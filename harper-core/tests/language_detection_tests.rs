...
fn test_detection(text: &str, expected_dialect: Dialect) {
    ...
    let detected = registry.detect_language(text, Dialect::American);
...
fn test_empty_file_defaults_to_english() {
    let registry = LanguageDetectionRegistry::new();
    let detected = registry.detect_language("", Dialect::American);
...
fn test_performance_long_document() {
    ...
    let detected = registry.detect_language(&long_german, Dialect::American);
...
fn test_edge_case_single_word() {
    test_detection("Hund", Dialect::American);
    test_detection("Dog", Dialect::American);
}

#[test]
fn test_edge_case_numbers_and_punctuation() {
    test_detection("123 456 789", Dialect::American);
    test_detection("... --- !!!", Dialect::American);
}
...
#[test]
fn test_portuguese_text_detects_portuguese() {
    test_detection(
        "O João foi à cidade de São Paulo e comprou pão para a família.",
        Dialect::Portuguese,
    );
}

#[test]
fn test_detected_english_preserves_preferred_variant() {
    let registry = LanguageDetectionRegistry::new();

    assert_eq!(
        registry.detect_language(
            "The neighbours organised the colour chart in the foyer.",
            Dialect::British,
        ),
        Dialect::British
    );
...
#[test]
fn test_detected_german_preserves_preferred_variant() {
    let registry = LanguageDetectionRegistry::new();
